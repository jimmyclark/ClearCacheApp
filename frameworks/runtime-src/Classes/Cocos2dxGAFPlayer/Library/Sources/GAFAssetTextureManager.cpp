...
void GAFAssetTextureManager::loadImages(const std::string& dir, GAFTextureLoadDelegate_t delegate, cocos2d::ZipFile* bundle)
{
	...
	if (!m_atlasInfos.empty())
	{
		for (unsigned int i = 0; i < m_atlasInfos.size(); ++i)
		{
            ...
            auto szPath = dir.substr(0, dir.rfind('/')+1)+source;
            auto textureCache = cocos2d::Director::getInstance()->getTextureCache();
            auto texture = textureCache->addImage(szPath);
            if (!texture)
            {
                CCLOG("GAF loadImages faild! dir=%s",dir.c_str());
            	return;
            }
            texture->retain();
            m_textures[info.id] = texture;
            m_memoryConsumption+=texture->getPixelsWide() * texture->getPixelsHigh() * texture->getBitsPerPixelForFormat() / 8;
