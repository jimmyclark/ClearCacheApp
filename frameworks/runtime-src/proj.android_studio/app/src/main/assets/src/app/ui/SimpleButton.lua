...
function SimpleButton:ctor(params, tag)
    ...
    self.touchHelper_ = poker.TouchHelper.new(self, self.onTouch_)
            ...
            if param.label then
                ...
                elseif type(param.label) == "table" then
                    if param.label.type == "ttf" then
                        self.labels_[state] = cc.ui.UILabel.new(param.label)
                    elseif param.label.type == "bmf" then
                        self.labels_[state] = cc.ui.UILabelBMFont.new(param.label)
                    end
                else
                    self.labels_[state] = param.label
                end
...
function SimpleButton:label(lb)
    for k,v in pairs(self.labels_) do
        v:setString(lb)
        v:setPositionX(v:getPositionX() - 22)
    end
    return self
end
...
function SimpleButton:onTouch_(evt, isTouchInSprite)
    if self:isVisible() and self:getParent() and self.isEnabled_ then
        if evt == poker.TouchHelper.TOUCH_BEGIN then
            self:changeState_("down")
        elseif evt == poker.TouchHelper.TOUCH_MOVE then
            if isTouchInSprite then
                self:changeState_("down")
            else
                self:changeState_("up")
            end
        elseif evt == poker.TouchHelper.TOUCH_END then
            self:changeState_("up")
        elseif evt == poker.TouchHelper.CLICK then
